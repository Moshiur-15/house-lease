...
const ManagePropertieModal = ({ propertyId, onDelete }) => {
  const handleDelete = async () => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/seller/property?id=${propertyId}`);
  ...
  return (
    <div className="absolute right-0 mt-2 w-32 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 z-50">
      <ul className="py-1">
        ...
        <Link href={`/dashboard/managePropertie/${propertyId}`} className="flex items-center gap-2 px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
...
export default ManagePropertieModal;
